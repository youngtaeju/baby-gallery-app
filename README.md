...
- `editor` 권한 사용자만 갤러리 상단에서 사진·동영상 업로드 가능
- 한 번에 최대 200개를 선택하고 SHA-256 중복 조회 후 필요한 파일만 전송
- tus 1.0과 5MB 청크를 사용하며 앱 재시작 후에도 서버 오프셋부터 재개
- 전송할 원본과 대기열은 앱 전용 support directory에 보관하고 완료 시 정리
- 앱이 백그라운드로 전환되거나 로그아웃하면 전송을 멈추고 포그라운드 복귀 시 재개
- 화면이 꺼진 동안의 백그라운드 업로드는 지원하지 않음
